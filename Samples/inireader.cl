...
else if not file_exists(elementAt(args, 0))
    log("File \"{elementAt(args, 0)}\" does not exist.")
    stop
endif

# Read the file into a string.
file is file_read(elementAt(args, 0))
...
repeat for len(lines) times as i
    line is trim(elementAt(lines, i))
    ...
    # If the line starts with a '[' we know it is a section
        ...
        set(get(ini, currentSect), trim(elementAt(splitLine, 0)), trim(join(range(splitLine, 1, len(splitLine)), "=")))
    endif
endrep
